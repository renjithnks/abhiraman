/* global abhiramanIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'abhiraman-supports-dark-theme' ) ) {
	abhiramanDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function abhiramanDarkModeEditorInit( attempt ) {
	...
	if ( abhiramanIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						abhiramanDarkModeEditorInit( attempt + 1 );
					},
