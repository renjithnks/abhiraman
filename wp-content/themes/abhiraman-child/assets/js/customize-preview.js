/* global abhiraman_childGetHexLum, jQuery */
( function() {
	// Add listener for the "background_color" control.
	wp.customize( 'background_color', function( value ) {
		value.bind( function( to ) {
			var lum = abhiraman_childGetHexLum( to ),
				isDark = 127 > lum,
				...
				stylesheetID = 'abhiraman_child-customizer-inline-styles',
				stylesheet,
				styles;
			...
			if ( ! stylesheet.length ) {
				jQuery( '#abhiraman_child-style-inline-css' ).after( '<style id="' + stylesheetID + '"></style>' );
				stylesheet = jQuery( '#' + stylesheetID );
			}
