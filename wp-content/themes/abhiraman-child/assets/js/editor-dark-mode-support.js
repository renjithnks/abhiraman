/* global abhiraman_childIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'abhiraman_child-supports-dark-theme' ) ) {
	abhiraman_childDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function abhiraman_childDarkModeEditorInit( attempt ) {
	...
	if ( abhiraman_childIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						abhiraman_childDarkModeEditorInit( attempt + 1 );
					},
